...
# Deliberately not the real brand: a distinctive value proves the pages render
# whatever SITE_NAME holds, rather than passing because someone hardcoded the
# product's name into a template.
BRAND = "Testbrand Messaging"


class TestBrandName:
    @override_settings(SITE_NAME=BRAND)
    ...
    def test_brand_appears_in_the_page_title(self, client: Client) -> None:
        body = client.get(reverse("accounts:login")).content.decode()
        assert f"<title>Sign in &middot; {BRAND}</title>" in body
