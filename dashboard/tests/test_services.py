...
class TestDailyActivity:
    def test_quiet_days_are_present_as_zeros(self, launched_campaign, make_message) -> None:
        ...
        activity = services.daily_activity(services.ReportPeriod.last_days(7))

        assert len(activity) == 7
        assert [day.total for day in activity[:-1]] == [0] * 6
        assert activity[-1].total == 1

    def test_the_buckets_partition_the_messages(self, launched_campaign, make_message) -> None:
        campaign = launched_campaign()
        for status in (
        ...
        today = services.daily_activity(services.ReportPeriod.last_days(1))[0]
    ...
    def test_messages_outside_the_period_are_excluded(self, launched_campaign, make_message) -> None:
        campaign = launched_campaign()
...
class TestOverview:
    def test_delivery_rate_is_measured_against_what_the_provider_accepted(
        self, launched_campaign, make_message
    ) -> None:
        """
        ...
        overview = services.overview(services.ReportPeriod.last_days(7))

        assert overview.messages == 10
        assert overview.reached == 2
        assert overview.delivery_rate == 50.0

    def test_read_counts_as_delivered(self, launched_campaign, make_message) -> None:
        campaign = launched_campaign()
    ...
    def test_rates_are_zero_rather_than_undefined_when_nothing_was_sent(self) -> None:
        overview = services.overview(services.ReportPeriod.last_days(7))
    ...
    def test_failure_rate_is_measured_against_every_recipient(
        ...
        assert services.overview(services.ReportPeriod.last_days(7)).failure_rate == 25.0

    def test_consent_movement_is_reported(self, make_contact) -> None:
...
class TestCampaignPerformance:
    def test_only_launched_campaigns_appear(self, make_campaign, launched_campaign) -> None:
        """A draft has no performance to report, so it is not a row."""
        make_campaign("Still a draft")
        launched_campaign("Actually sent")

        rows = services.campaign_performance(services.ReportPeriod.last_days(7))

        assert [row.name for row in rows] == ["Actually sent"]

    def test_a_campaign_launched_before_the_period_is_excluded(self, launched_campaign) -> None:
        launched_campaign("Old news", days_ago=60)
        assert services.campaign_performance(services.ReportPeriod.last_days(7)) == []
    ...
    def test_rates_come_from_the_shared_stats_object(
        ...
        row = services.campaign_performance(services.ReportPeriod.last_days(7))[0]
...
class TestActiveCampaigns:
    def test_sending_and_paused_campaigns_are_listed(self, launched_campaign) -> None:
        ...
        names = {row.name for row in services.active_campaigns()}

        assert names == {"Sending", "Paused"}

    def test_progress_is_reported_per_campaign(self, launched_campaign, make_message) -> None:
        campaign = launched_campaign("Sending", status=CampaignStatus.PROCESSING)
        make_message(campaign, status=MessageStatus.DELIVERED)
        make_message(campaign, status=MessageStatus.QUEUED)

        row = services.active_campaigns()[0]
...
class TestFailureReasons:
    def test_failures_are_grouped_by_the_provider_error(
        ...
        reasons = services.failure_reasons(services.ReportPeriod.last_days(7))
    ...
    def test_the_number_of_affected_campaigns_is_reported(
        ...
        reason = services.failure_reasons(services.ReportPeriod.last_days(7))[0]
    ...
    def test_successful_messages_are_never_counted_as_failures(
        ...
        assert services.failure_reasons(services.ReportPeriod.last_days(7)) == []

    def test_recent_failures_span_campaigns(self, launched_campaign, make_message) -> None:
        ...
        assert len(services.recent_failures()) == 2


class TestConsentSummary:
    def test_eligibility_matches_the_consent_rule(self, make_contact) -> None:
        ...
        summary = services.consent_summary()
    ...
    def test_the_opt_in_source_is_reported(self, make_contact) -> None:
        make_contact("Consenting", opted_in=True)

        sources = dict(services.consent_summary().by_opt_in_source)

        assert sources == {"Entered manually by an operator": 1}

    def test_an_empty_list_reports_zero_rather_than_dividing_by_it(self) -> None:
        assert services.consent_summary().opt_in_rate == 0.0
