...
class TestMockProvider:
    def test_sync_is_refused_rather_than_reporting_zero_templates(self) -> None:
        """
...
class TestSync:
    def test_a_template_is_created_from_the_provider(self, meta, http) -> None:
        stub(http, meta_template())

        assert sync_templates_from_provider() == 1
    ...
    def test_the_variables_are_read_out_of_the_body(self, meta, http) -> None:
        stub(http, meta_template())
    ...
    def test_re_syncing_updates_rather_than_duplicating(self, meta, http) -> None:
        stub(http, meta_template())
        sync_templates_from_provider()
    ...
    def test_the_same_name_in_another_language_is_a_separate_template(self, meta, http) -> None:
        stub(http, meta_template(), meta_template(language="ne", id="222"))

        assert sync_templates_from_provider() == 2
        assert MessageTemplate.objects.count() == 2

    def test_a_template_with_no_name_is_skipped_not_stored(self, meta, http) -> None:
        stub(http, meta_template(name=""))

        assert sync_templates_from_provider() == 0
        assert MessageTemplate.objects.count() == 0

    def test_the_sync_is_audited(self, meta, http, operator) -> None:
        stub(http, meta_template())

        sync_templates_from_provider(user=operator)

        entry = AuditLog.objects.get(action=AuditAction.TEMPLATES_SYNCED)
        assert entry.user == operator
        assert entry.metadata["count"] == 1

    def test_a_provider_error_raises_rather_than_reporting_zero(self, meta, http) -> None:
        http.add(responses.GET, TEMPLATES_URL, json={"error": {"code": 190}}, status=401)

        with pytest.raises(ProviderError):
            sync_templates_from_provider()

    def test_nothing_is_written_when_the_provider_fails(self, meta, http) -> None:
        http.add(responses.GET, TEMPLATES_URL, json={"error": {"code": 190}}, status=401)
...
class TestApprovalBelongsToMeta:
    def test_only_what_meta_reports_as_approved_becomes_approved(self, meta, http) -> None:
        stub(
            http,
    ...
    def test_a_template_meta_has_not_approved_stays_unusable(self, meta, http) -> None:
    ...
    def test_a_local_template_is_replaced_by_metas_version_of_it(self, meta, http) -> None:
        """
    ...
    def test_an_unfamiliar_state_never_becomes_approved(self, meta, http) -> None:
